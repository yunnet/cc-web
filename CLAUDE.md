...
A `Stop` hook (every turn's end, with `last_assistant_message`; the server forwards its first 300 chars) is the precise "finished" signal: measured on 2.1.278 it fires with the ✳ title on a real answer — sometimes just before it — and not while Claude waits for a permission, an AskUserQuestion or a plan approval (those send `permission_prompt` ~6s later). Both views hand every `hook_event` to `sessionTabManager.hookEvent` (the ExitPlanMode plan modal stays in `app.js`); `turnFinished` puts the ✓ up at once for a tab nobody is looking at and flags it (`_doneByStop`) so the ✳ title that follows does not redo it; the 8s grace timer stays as the fallback for a Claude without the hook, and a tab watched when the turn ended keeps it as before. The notification body is the reply's first line, else the topic.

Links: Claude runs with `FORCE_HYPERLINK=1` and prints OSC 8 hyperlinks (none without it). Both terminals set `linkHandler` → `app.openTerminalLink`, gated by `src/public/terminal-links.js` (http(s) → new tab; `file:` on this host → a plan file via `planUrl`, shared with `registerPlanLinks`, else `fileExplorer.openFile`; anything else ignored). This replaces xterm's default handler, which prompts and refuses `file:`.
